                    ...
                    SELECT JSON_ARRAYAGG(
                        JSON_OBJECT(
                            ...
                            'index_type', CASE
                                ...
                                ELSE 'INDEX'
                            END,
                            'column_names', GROUP_CONCAT(s.COLUMN_NAME ORDER BY s.SEQ_IN_INDEX SEPARATOR ','),
            ...
            DB::transaction(function () use ($tableData, $dbId) {
                try {
                    ...
                    foreach ($columns as $column) {
                        ...
                        $dataType = $this->formatMySqlDataType((object)$column);
                    ...
                    Log::error('Erreur lors du traitement de la table MySQL', [
                        'table' => $tableData->table_name,
                        'error' => $e->getMessage(),
                        'trace' => $e->getTraceAsString()
                    ]);
                ...
                SELECT
                    ...
                    COALESCE(
                        (SELECT COMMENT 
                         ...
                         WHERE t2.TRIGGER_NAME = t.TRIGGER_NAME 
                         AND t2.TRIGGER_SCHEMA = t.TRIGGER_SCHEMA 
                         LIMIT 1), 
                        ''
                    ) AS description
